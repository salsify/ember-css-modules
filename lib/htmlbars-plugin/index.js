...
module.exports = class ClassTransformPlugin {
  ...
  dynamicLocalPath(node) {
    ...
    let hash = this.builders.hash([this.builders.pair('from', this.unboundStyles())]);
  ...
  unboundStyles() {
    let stylesPath = this.builders.path('__styles__');
    let unboundPath = this.builders.path('unbound');
  ...
  staticLocalPath(node) {
    ...
    let builder = typeof node.chars === 'string' ? 'mustache' : 'sexpr';
    let classes = locals.split(' ');

    return classes.filter(function(local) {
      return local.trim().length > 0;
    }).map(function(local) {
      let unboundPath = this.builders.path('unbound');
      let stylePath = this.builders.path('__styles__.' + local.trim());
