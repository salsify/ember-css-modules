...
const isModuleUnification = require('./utils/is-module-unification');
...
module.exports = class ModulesPreprocessor {
  ...
  getModulesTree() {
    if (!this._modulesTree) {
      const isMU = isModuleUnification(this.owner.parent);
      const belongsToAddon = this.owner.belongsToAddon();

      let inputRoot = belongsToAddon
      ...
      let outputRoot = isMU
        ? path.join(
            ...
            (belongsToAddon ? this.owner.parent : this.owner).treePaths.src
          )
        : this.owner.belongsToAddon()
