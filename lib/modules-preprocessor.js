...
module.exports = class ModulesPreprocessor {
  ...
  getModulesTree() {
    if (!this._modulesTree) {
      ...
      let modulesSources = new ModulesFunnel(inputRoot, {
...
class ModulesFunnel extends Funnel {
  constructor(input, options) {
    super(input, options);
    this.parentName = options.parentName;
    this.destDir = options.outputRoot;
    this.destDirConfigured = false;
  }

  build() {
    if (!this.destDirConfigured) {
      this.destDirConfigured = true;
      if (!fs.existsSync(`${this.inputPaths[0]}/${this.parentName}`)) {
