...
ModulesPreprocessor.prototype.toTree = function(inputTree, path) {
  ...
  var merged = new MergeTrees([inputTree, this.getModulesTree()], { overwrite: true });
...
ModulesPreprocessor.prototype.getModulesTree = function() {
  if (!this._modulesTree) {
    var inputRoot = this.owner.belongsToAddon() ? this.owner.getParentAddonTree() : this.owner.app.trees.app;
    var outputRoot = (this.owner.belongsToAddon() ? this.owner.getAddonModulesRoot() : '');

    var modulesSources = new Funnel(inputRoot, {
      include: ['**/*.' + this.owner.getFileExtension()],
      destDir: outputRoot + this.owner.getOwnerName(),
    });

    this._modulesTree = require('broccoli-css-modules')(modulesSources, {
