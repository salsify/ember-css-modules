...
function resolveRelativePath(importPath, fromFile, options) {
  var absolutePath = path.resolve(path.dirname(fromFile), importPath);
...
function resolveLocalPath(importPath, fromFile, options) {
  ...
  var absolutePath = path.resolve(prefix, importPath);
  return internalDep(absolutePath, options);
}
...
function resolveExternalPath(importPath, options) {
  ...
  var absolutePath = path.resolve(addonTreePath, pathWithinAddon);
  var keyPath = path.join('modules', addonName, pathWithinAddon);
