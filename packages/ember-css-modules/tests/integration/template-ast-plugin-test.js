...
module('Integration | Template AST Plugin', function (hooks) {
  ...
  hooks.beforeEach(function () {
    ...
    this.owner.register(
      'component:x-div',
      Component.extend({ attributeBindings: ['data-test-value'] })
    );
    this.owner.register(
  ...
  testTransformation(
    'appending to an unquoted string literal class attribute',
    {
      input: 'class=x local-class="foo"',
  ...
  testTransformation(
    'appending to an unquoted string literal class attribute with multiple classes',
    {
      input: 'class=x local-class="foo bar"',
  ...
  function testTransformation(
    ...
    test(title, async function (assert) {
      ...
      const assertTransforms = async (template, type, input, output) => {
        ...
        assert.ok(
          this.element.querySelector(selector),
          'Actual output contains verification selector'
        );
